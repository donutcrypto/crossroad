...
contract("TestCrossroad_PostDepositBnbPayFee", accounts => {
  let feeToken;
  let feeAutoBuyer;
  ...
  it("should post deposit bnb", async () => {
    ...
    const initBnbAmount = BigInt(await web3.eth.getBalance(accounts[0]));
    ...
    assert.equal(posterBnbAmount,initBnbAmount-feeAmount-inputAmountReward-inputAmountIn-(feeGasUsed+postGasUsed)*gasPrice, "Mismatch: posterBnbAmount");
    assert.equal(crossroadBnbAmount,inputAmountIn, "Mismatch: crossroadBnbAmount");
...
contract("TestCrossroad_PostDepositBnbBuyFee", accounts => {
  let feeToken;
  let feeAutoBuyer;
  ...
  it("should post deposit bnb", async () => {
    ...
    // post order
    const postTx = await crossroad.postOrderInBnbBuyFee(
      testTokenB.address, // _tokenOut,
      inputAmountIn, // _amountIn,
      inputAmountOut, // _amountOut,
      now+1200n, // _expiryTime,
      inputAmountReward, // _rewardAmount
      {from: accounts[0], value: (feeAmount+inputAmountReward+inputAmountIn).toString()}
      );
    const postGasUsed = BigInt(postTx.receipt.gasUsed);
    console.log('post gas: '+postGasUsed.toString());
    ...
    const posterBnbAmount = BigInt(await web3.eth.getBalance(accounts[0]));
    const crossroadBnbAmount = BigInt(await web3.eth.getBalance(crossroad.address));
    assert.equal(posterBnbAmount,initBnbAmount-feeAmount-inputAmountReward-inputAmountIn-postGasUsed*gasPrice, "Mismatch: posterBnbAmount");
    assert.equal(crossroadBnbAmount,inputAmountIn, "Mismatch: crossroadBnbAmount");
...
contract("TestCrossroad_PostDepositBnbPayFee_Renew", accounts => {
  let feeToken;
  let feeAutoBuyer;
  ...
  it("should post deposit bnb", async () => {
    ...
    const posterBnbAmount = BigInt(await web3.eth.getBalance(accounts[0]));
    const crossroadBnbAmount = BigInt(await web3.eth.getBalance(crossroad.address));
    assert.equal(posterBnbAmount,initBnbAmount-feeAmount-inputAmountReward-inputAmountIn-(feeGasUsed+postGasUsed+renewGasUsed)*gasPrice, "Mismatch: posterBnbAmount");
    assert.equal(crossroadBnbAmount,inputAmountIn, "Mismatch: crossroadBnbAmount");
...
contract("TestCrossroad_PostDepositBnbPayFee_Cancel", accounts => {
  let feeToken;
  let feeAutoBuyer;
  ...
  it("should post deposit bnb", async () => {
    ...
    const posterBnbAmount = BigInt(await web3.eth.getBalance(accounts[0]));
    const crossroadBnbAmount = BigInt(await web3.eth.getBalance(crossroad.address));
    assert.equal(posterBnbAmount,initBnbAmount-feeAmount-inputAmountReward-(feeGasUsed+postGasUsed+cancelGasUsed)*gasPrice, "Mismatch: posterBnbAmount");
    assert.equal(crossroadBnbAmount,0n, "Mismatch: crossroadBnbAmount");
