...
contract("TestCrossroad_PostDepositTokenPayFee_FillOver2", accounts => {
  ...
  it("should post deposit token", async () => {
    ...
    const deposit2Tx = await testCallee.deposit(
      {from: accounts[2], value: actualAmountOut2.toString()}
      )
    ...
    assertEqBigInt(filler2BnbAmount,initFiller2BnbAmount-actualAmountOut2-(deposit2GasUsed+fill2GasUsed)*gasPrice, "Mismatch: filler2BnbAmount");
    assertEqBigInt(crossroadBnbAmount,0n, "Mismatch: crossroadBnbAmount");
