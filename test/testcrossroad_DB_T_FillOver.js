...
contract("TestCrossroad_PostDepositBnbPayFee_FillOver1", accounts => {
  let feeToken;
  let feeAutoBuyer;
  ...
  it("should post deposit token", async () => {
    ...
    assert.equal(posterBnbAmount,initPosterBnbAmount-feeAmount-inputAmountReward-inputAmountIn-(feeGasUsed+postGasUsed)*gasPrice, "Mismatch: posterBnbAmount");
    assert.equal(fillerBnbAmount,initFillerBnbAmount+inputAmountIn-(fillGasUsed)*gasPrice, "Mismatch: fillerBnbAmount");
...
contract("TestCrossroad_PostDepositBnbPayFee_FillOver2", accounts => {
  let feeToken;
  let feeAutoBuyer;
  ...
  it("should post deposit token", async () => {
    ...
    const crossroadBnbAmount = BigInt(await web3.eth.getBalance(crossroad.address));
    assert.equal(posterBnbAmount,initPosterBnbAmount-feeAmount-inputAmountReward-inputAmountIn-(feeGasUsed+postGasUsed)*gasPrice, "Mismatch: posterBnbAmount");
    assert.equal(filler1BnbAmount,initFiller1BnbAmount+inputAmountIn*25n/100n-(fill1GasUsed)*gasPrice, "Mismatch: filler1BnbAmount");
    assert.equal(filler2BnbAmount,initFiller2BnbAmount+inputAmountIn*75n/100n-(fill2GasUsed)*gasPrice, "Mismatch: filler2BnbAmount");
    assert.equal(crossroadBnbAmount,0n, "Mismatch: crossroadBnbAmount");
